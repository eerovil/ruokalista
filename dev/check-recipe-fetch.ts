...
test("a challenged K-Ruoka page falls back to its read-only recipe API", async () => {
  const pageUrl = "https://www.k-ruoka.fi/reseptit/helppo-texmex-broilerilasagne";
  const buildUrl =
    "https://www.k-ruoka.fi/kr-api/v1/recipe/helppo-texmex-broilerilasagne";
  const storesUrl = "https://www.k-ruoka.fi/kr-api/stores/";
  const apiUrl =
    "https://www.k-ruoka.fi/kr-api/v1/recipe/helppo-texmex-broilerilasagne?storeId=N106";
  const requests: Array<{ url: string; init: RequestInit }> = [];
  const apiRecipe = {
    recipe: {
      name: { fi: "Helppo texmex-broilerilasagne" },
      description: { fi: "Helppo yhden kastikkeen lasagne." },
      servingCountLabel: "6",
      servingCountUnit: { fi: "annosta" },
      ingredients: [
        {
          amount: { fi: "1" },
          unit: { fi: "rs" },
          additionalInfo: { fi: "(400 g)" },
          productSpelling: { fi: "kanan fileesuikaleita" },
        },
        {
          amount: { fi: "1" },
          unit: null,
          additionalInfo: { fi: "" },
          productSpelling: { fi: "sipuli" },
        },
      ],
      instructions: [
        { instruction: { fi: "Ruskista broileri ja sipuli." } },
        { instruction: { fi: "Kypsennä lasagne uunissa." } },
      ],
      pictures: [{ url: "https://public.keskofiles.com/f/recipe/helppotexmex" }],
    },
  };
  const responses: Record<string, Response> = {
    [pageUrl]: htmlResponse("<p>Just a moment...</p>", {
      status: 403,
      headers: { "cf-mitigated": "challenge" },
    }),
    [buildUrl]: new Response(JSON.stringify({ error: { message: "reload" } }), {
      status: 409,
      headers: { "Content-Type": "application/json", "K-Ruoka-Build": "32254" },
    }),
    [storesUrl]: new Response(JSON.stringify({ results: [{ id: "N106" }] }), {
      headers: { "Content-Type": "application/json" },
    }),
    [apiUrl]: new Response(JSON.stringify(apiRecipe), {
      headers: { "Content-Type": "application/json; charset=utf-8" },
    }),
  };

  const page = await fetchRecipePage(pageUrl, async (url, init) => {
  ...
  assert.equal(page.title, "Helppo texmex-broilerilasagne");
  assert.equal(page.structured, true);
  assert.equal(page.url, pageUrl);
  assert.equal(
    page.sourceText,
    [
      "Helppo texmex-broilerilasagne",
      "",
      "Helppo yhden kastikkeen lasagne.",
      "",
      "Annoksia: 6 annosta",
      "",
      "Ainekset:",
      "1 rs (400 g) kanan fileesuikaleita",
      "1 sipuli",
      "",
      "Valmistus:",
      "1. Ruskista broileri ja sipuli.",
      "2. Kypsennä lasagne uunissa.",
    ].join("\n"),
  );
  assert.deepEqual(page.imageUrls, [
    "https://public.keskofiles.com/f/recipe/helppotexmex",
  ]);
  assert.deepEqual(
    requests.map((request) => request.url),
    [pageUrl, buildUrl, storesUrl, apiUrl],
  );
  assert.equal(
    new Headers(requests[0]?.init.headers).get("User-Agent"),
    "Ruokalista/1.0 (recipe import; +https://ruokalista.vilpponen.fi)",
  );
  assert.equal(new Headers(requests[0]?.init.headers).get("Sec-Fetch-Site"), null);
  assert.equal(new Headers(requests[1]?.init.headers).get("Sec-Fetch-Site"), "cross-site");
  assert.equal(new Headers(requests[2]?.init.headers).get("X-K-Build-Number"), "32254");
  assert.equal(new Headers(requests[3]?.init.headers).get("X-K-Build-Number"), "32254");
});

test("a changed K-Ruoka API preserves the ordinary page refusal", async () => {
  const pageUrl = "https://www.k-ruoka.fi/reseptit/muuttunut";
  const buildUrl = "https://www.k-ruoka.fi/kr-api/v1/recipe/muuttunut";
  const storesUrl = "https://www.k-ruoka.fi/kr-api/stores/";
  const apiUrl = "https://www.k-ruoka.fi/kr-api/v1/recipe/muuttunut?storeId=N106";

  await assert.rejects(
    fetchRecipePage(pageUrl, fetcherFor({
      [pageUrl]: htmlResponse("<p>Just a moment...</p>", { status: 403 }),
      [buildUrl]: new Response("reload", {
        status: 409,
        headers: { "K-Ruoka-Build": "32254" },
      }),
      [storesUrl]: new Response(JSON.stringify({ results: [{ id: "N106" }] }), {
        headers: { "Content-Type": "application/json" },
      }),
      [apiUrl]: new Response(JSON.stringify({
        recipe: {
          name: { fi: "Muuttunut resepti" },
          ingredients: [
            { amount: { fi: "1" }, productSpelling: { fi: "sipuli" } },
            { amount: { fi: "2" }, productSpelling: "new shape" },
          ],
          instructions: [{ instruction: { fi: "Pilko sipuli." } }],
        },
      }), { headers: { "Content-Type": "application/json" } }),
    })),
    (error: unknown) =>
      error instanceof PageRefused &&
      error.reason === "unreachable" &&
      error.message === "Page answered 403.",
  );
});
