...
The remaining hand-written islands follow the same discipline because their
strings reach the browser without transpilation:
...
These hand-written islands remain ES5 (`var`, no arrow functions, no regular
expressions inside template strings). The editor's image-upload island in
...
queued-import submission (#263). `scripts/build-client.mjs` now builds two fixed
entries: intake at **ES5**, and the existing image splitter at **ES2019**. It
writes committed string modules under `src/generated/`; `intake-screens.ts`
embeds the intake string in the same place as before, without a new route or
framework. Run `npm run generate:client` after edits. `npm run check:client`
rejects either missing or stale output, and the DOM tsconfig checks both clients.
The splitter's generated bytes and API requirements are unchanged.

The intake form passes only three positive integer limits through escaped
`data-*` attributes, using the same constants as server validation. User text,
URLs, guidance, recipe identity and mode stay in ordinary form controls and are
read on submit; they are never interpolated into the script. No Worker-only
module or model SDK enters the browser bundle.

Keep the ES5-style control flow (`var` and functions) in this typed source:
esbuild's ES5 target prevents the minifier introducing modern syntax, but does
not generally lower arbitrary ES6 syntax to ES5. Type annotations do not change
that browser floor. See https://esbuild.github.io/content-types/#es5-is-not-supported-well.
Required APIs (`fetch`, `Promise`) and optional photo APIs are still detected at
runtime. Without the required APIs, the disabled button and JavaScript guidance
remain; without image decoding, pasted/linked input still works. The serial image
chain, early bitmap/canvas release, page order and limits, error wording, and
queued-import/edit request protocol stay the same. Existing intake and edit
browser tests exercise the generated program, not a retained legacy copy.
