...
- **`npm run check` runs the dev checks under Node's strip-only TypeScript.**
  ...
  is the one that has already bitten — typechecks fine, passes under `tsx`, and
  fails the check with `ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX` in any `src/` module a
  `dev/check-*.ts` imports. See [testing](docs/codebase/testing.md).
...
- **Link implementation PRs without closing their issues.** Use exactly one
  `Issue: #N` line in the PR body; never use GitHub's `Closes`, `Fixes` or
  `Resolves` keywords. A merge means the code is on `main`; the issue closes
  only after production deploy and live verification succeed.
