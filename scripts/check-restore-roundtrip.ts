...
try {
  ...
  // Two categories on one dish: a restore that dropped the table, or kept only
  // one row per recipe, would leave the lasagne unfindable rather than fail
  // loudly (#196).
  const categories = target.recipe_category
  ...
  if (
    target.intake_job[0]?.source_text !== "Uunikaali" ||
    target.intake_job[0]?.status !== "failed" ||
    target.intake_job[0]?.import_guidance !== "Pidä kaalikerrokset erillään."
  ) {
  ...
  // #244: the only record of which S-list text rows are this app's to delete.
  // Lose it in a restore and every one of them is stranded on the phone.
  const note = target.s_ostoslista_sent_note[0];
  if (
    note?.household_id !== 1 ||
    note.row_key !== "1" ||
    note.note !== "öljy — 5 dl"
  ) {
    throw new Error("round-trip did not preserve the sent S-ostoslista note");
  }
  if (
    target.planned_batch[0]?.recipe_id !== 3 ||
    target.planned_batch[0]?.multiplier !== 1.5 ||
    target.planned_batch[0]?.instance_key !== "batch-instance-1"
  ) {
